---
name: starlight-docs
description: Author and edit Astro Starlight documentation on the current stack (Astro 7, Starlight 0.41+, Sätteri), and make it machine-readable for AI agents. Covers the full Expressive Code meta-string syntax (line highlighting, ranges, labelled markers, diff syntax, collapsible sections, line numbers, frames, titles), Starlight's built-in MDX components (Tabs, Steps, FileTree, Card, CardGrid, LinkCard, LinkButton, Aside, Badge, Icon, Code), the Sätteri Markdown pipeline and why remark/rehype plugins no longer run, LLM-friendliness (llms.txt, raw Markdown routes, page action buttons, rel=alternate head tags), and og:image share cards — the last two both being things every site needs and Starlight does not generate. Use whenever writing or editing .md, .mdx or .mdoc files in a Starlight project, whenever a site needs social share cards, OpenGraph images or link previews, whenever a code block needs more than a bare language tag, whenever content would suit a built-in component, and whenever touching astro.config.mjs or ec.config.mjs. Also use whenever setting up, reviewing or finishing a docs site, since share cards and the Markdown surface are missing by default and neither announces itself, when asked to make docs agent-friendly or LLM-readable, when a remark or rehype plugin stops working, when upgrading Astro or Starlight, or when a code block renders wrongly.
---
...
| `references/llm-friendly.md` | Making the site machine-readable: llms.txt, per-page `.md` routes, page action buttons, MCP, discovery signposting, writing prose that survives Markdown conversion. **Every site needs this** — check before calling a site finished. |
...
7. **Plugin-injected routes are invisible to other plugins.** Pages generated by `starlight-openapi`, `starlight-pydocs` and friends are injected routes, not content collection entries. `starlight-links-validator` reports every link into them as broken until you `exclude` their base; `starlight-llms-txt` omits them from `llms-full.txt` entirely — bridge that with `optionalLinks`; and `starlight-page-actions` writes no raw `.md` for them while still rendering its buttons on them, so Copy Markdown 404s on every generated page. None is obvious from the error message, the last two are silent, and only the first fails a build. Test a generated page, not just a hand-written one.
...
10. **Every site gets a Markdown surface.** Documentation is now read by agents at least as often as by people, and an HTML-only site makes both of them work for it: the agent scrapes a rendered page, and the reader has nothing to paste. This is not an on-request extra — treat it like share cards. A site needs raw `.md` per page, `llms.txt` at the root, a visible link to it, and the `rel="alternate"` head tag. If a project has none of that, say so and offer — `starlight-llms-txt` plus `starlight-page-actions` is an afternoon, and the head tag and the signpost are the cheap half that make the rest count. See `references/llm-friendly.md`.
...
**Setting up, reviewing or finishing a site?** Two things are almost always missing, and neither announces itself. **Share cards**: Starlight generates every OpenGraph tag except `og:image`, so the omission stays invisible until a link unfurls grey in Slack — `astro-og-canvas` plus a `Head` override, recipe in `references/share-cards.md`. **A Markdown surface**: `grep` the built HTML for `rel="alternate"` and `ls dist/llms*.txt`; empty results mean every agent reading these docs is scraping HTML. Recipe in `references/llm-friendly.md`. Raise both before calling a site done, whether or not anyone asked.

**Asked to make the docs agent-friendly?** Two plugins and a signpost: `starlight-llms-txt` for site-level indexes and `starlight-page-actions` for copy/open buttons plus raw `.md` routes — then a visible link to `/llms.txt`. **Not `starlight-contextual-menu`**, which is stuck on Astro 5 despite being the most-recommended option. Generating the files without linking to them is the usual failure; the runner-up is stopping before the `<link rel="alternate" type="text/markdown">` head tag, which no plugin emits for you. On a site hosting several products, `customSets` is required rather than optional, or retrieval answers about the wrong product. On a site with plugin-generated pages, see non-negotiable 7 — the copy buttons are broken there by default. MCP is a later step, and extending an existing product MCP usually beats a docs-only one. See `references/llm-friendly.md`.
